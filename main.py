...
from KG.chunk import split_data_from_file, load_neo4j_graph
import json
...
for name in file_names:
    ...
    with open(file, 'r', encoding='utf-8') as f:
        ...
        if name == "Battle_of_Waterloo":
            create_nodes(graph=graph, data=data, node_label="Event", node=name)
        else:
            create_nodes(graph=graph, data=data, node_label="Person", node=name)
    # Ingest Chunks
    ingest_Chunks(graph=graph, chunks=chunks, node=name, node_label='Chunk')
...
embed_text(graph = graph, OPENAI_API_KEY=openAI_api, OPENAI_ENDPOINT=openAI_endpoint, name='Chunk')