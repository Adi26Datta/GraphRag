...
def create_nodes(graph, data: dict, node_label: str, name: str):
    main_node_query = f"""
    MERGE (main:{node_label} {{name: $name}})
    """
    graph.query(main_node_query, params={"name": name})

    # Create section nodes only (without relationships)
    for subhead, _ in data.items():
        query = f"""
        MERGE (s:Section {{subhead: $subhead, name: $name}})
        """
        params = {"subhead": subhead,"name": name}
        graph.query(query, params=params)

# 2. Add Chunks
def ingest_Chunks(graph, chunks, name, node_label):
    """
    ...
    MERGE (mergedChunk:{node_label} {{chunkId: $chunkParam.chunkId}})
        ON CREATE SET
            ...
            mergedChunk.subhead = $chunkParam.subhead, 
            mergedChunk.formItem = $chunkParam.formItem, 
            mergedChunk.chunkSeqId = $chunkParam.chunkSeqId,
            mergedChunk.name = $name
    RETURN mergedChunk
    """
    node_count = 0
    for chunk in chunks:
        print(f"Creating `:{node_label}` node for chunk ID {chunk['chunkId']}")
        graph.query(merge_chunk_node_query, params={'chunkParam': chunk, 'name': name})
        node_count += 1
...
def embed_text(graph, OPENAI_API_KEY, OPENAI_ENDPOINT, name):
    """
